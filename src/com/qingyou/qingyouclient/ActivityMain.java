...
public class ActivityMain extends FragmentActivity implements ActionBar.TabListener,
	...
	ServiceConnection mConnection = new ServiceConnection() {
		...
		public void onServiceConnected(ComponentName name, IBinder service) {
			...
			//启动登录窗口
			if (PrefConfig.instance(ActivityMain.this).getCfg("account").equals("")) {        
				Intent intent = new Intent();
			...
			if (intent.getAction().equals("qingyou.net.trans")) {
				...
				case HttpThread.NET_ORDER_REFRESH:
					if (fragments[ActivityMain.PAGE_1] != null) {
						((OrderListFragment)fragments[ActivityMain.PAGE_1]).setData();
					}
					if (fragments[ActivityMain.PAGE_2] != null) {
						((OrderListFragment)fragments[ActivityMain.PAGE_2]).setData();
					}
					if (fragments[ActivityMain.PAGE_3] != null) {
						((OrderListFragment)fragments[ActivityMain.PAGE_3]).setData();
					}
					if (fragments[ActivityMain.PAGE_4] != null) {
						((OrderListFragment)fragments[ActivityMain.PAGE_4]).setData();
					}
					break;
