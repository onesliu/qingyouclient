...
public class HttpUtility {
    ...
    public static String encodeUrl(HttpParameters parameters) throws UnsupportedEncodingException {
        if (parameters == null) {
            return "";
        }
     ...
     */
    public static UrlEncodedFormEntity getPostParamters(Bundle bundle) throws ProtocolException {
    ...
    public static String openUrl(Context context, String url, String method,
        ...
        try {
            ...
            if (statusCode != 200) {
				...
				ProtocolException dse = new ProtocolException("errno:" + errCode + " ," + err + ",statusCode:" + statusCode);
	...
	private static void parseCookies(HttpResponse response) {
		...
		for (int i = 0; i < respHeaders.length; i++) {
			//Log.v(respHeaders[i].getName() + ":" + respHeaders[i].getValue());
    ...
    public static HttpClient getNewHttpClient(Context context) {
        try {
            ...
            HttpConnectionParams.setConnectionTimeout(params, HttpUtility.SET_CONNECTION_TIMEOUT);
            HttpConnectionParams.setSoTimeout(params, HttpUtility.SET_SOCKET_TIMEOUT);
    ...
    public static HttpClient getHttpClient(Context context) {
        ...
        // Set the default socket timeout (SO_TIMEOUT) // in
        // milliseconds which is the timeout for waiting for data.
        HttpConnectionParams.setConnectionTimeout(httpParameters, HttpUtility.SET_CONNECTION_TIMEOUT);
        HttpConnectionParams.setSoTimeout(httpParameters, HttpUtility.SET_SOCKET_TIMEOUT);
    ...
    public static String encodeParameters(HttpParameters httpParams) {
        ...
        for (int loc = 0; loc < httpParams.size(); loc++) {
            ...
            try {
                buf.append(URLEncoder.encode(key, "UTF-8")).append("=")//.append(httpParams.getValue(key));
    ...
    /**
     * Base64 encode mehtod for request.Refer to weibo development
     * document.
     * 
     */
